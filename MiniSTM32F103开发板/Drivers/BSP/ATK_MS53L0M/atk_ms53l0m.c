...
#define ATK_MS53L0M_SENSOR_TYPE         0x0A    /* ATK-MS53L0M传感器类型 */
...
static uint8_t atk_ms53l0m_unpack_recv_data(uint16_t *dat)
{
    ...
    if (opt_type == 0x00)
    {
        ...
        frame_check_sum = atk_ms53l0m_crc_check_sum(&frame_buf[frame_head_index], dat_len + 8);   /* 计算CRC校验和 */
    ...
    {
        frame_check_sum = atk_ms53l0m_crc_check_sum(&frame_buf[frame_head_index], 6);    /* 计算CRC校验和 */
...
uint8_t atk_ms53l0m_read_data(uint16_t addr, uint8_t fun_code, uint8_t len, uint16_t *dat)
{
    ...
    atk_ms53l0m_uart_rx_restart();                  /* 准备重新开始接收新的一帧数据 */
    atk_ms53l0m_uart_send(buf, 9);                  /* 发送数据 */
    ret = atk_ms53l0m_unpack_recv_data(dat);        /* 解析应答数据 */
...
uint8_t atk_ms53l0m_write_data(uint16_t addr, uint8_t fun_code, uint8_t dat)
{
    ...
    atk_ms53l0m_uart_rx_restart();                  /* 准备重新开始接收新的一帧数据 */
    atk_ms53l0m_uart_send(buf, 10);                 /* 发送数据 */
    ret = atk_ms53l0m_unpack_recv_data(NULL);       /* 解析应答数据 */
 ...
 * @param       baudrate: ATK-MS53L0M UART通讯波特率
 *              id      : ATK-MS53L0M的设备ID
 * @retval      ATK_MS53L0M_EOK  : ATK-MS53L0M初始化成功，函数执行成功
 *              ATK_MS53L0M_ERROR: ATK-MS53L0M初始化失败，函数执行失败
 */
uint8_t atk_ms53l0m_init(uint32_t baudrate, uint16_t *id)
{
    uint8_t i;
    
    /* ATK-MS53L0M UART初始化 */
    atk_ms53l0m_uart_init(baudrate);
    
    /* 获取设备地址 */
    i = 0;
    while (atk_ms53l0m_read_data(0xFFFF, ATK_MS53L0M_FUNCODE_IDSET, 2, id) != ATK_MS53L0M_EOK)
    {
        delay_ms(100);
        if (++i == 5)
        {
...
/**
 * @brief       ATK-MS53L0M Normal工作模式获取测量值
 * @param       dat: 获取到的测量值
 * @retval      ATK_MS53L0M_EOK  : 获取测量值成功
 *              ATK_MS53L0M_ERROR: UART未接收到数据，获取测量值失败
 */
